...
const SeedCard = (props) => {
  ...
  const like = async (seed) => {
    ...
    let id = seed.id;
    const res = await axios.post("https://urarawin-worker.urarawin.workers.dev/api/sqlite/like", { id, userId });
    if (res.data) {
      message.info(t("成功"));
      seed.likes += 1;
    }
  ...
  const dislike = async (seed) => {
    ...
    let id = seed.id;
    const res = await axios.post("https://urarawin-worker.urarawin.workers.dev/api/sqlite/dislike", { id, userId });
    if (res.data) {
      message.info(t("成功"));
      seed.dislikes += 1;
    }
...
const Seed = () => {
  ...
  const search = async (value) => {
    const res = await axios.post("https://urarawin-worker.urarawin.workers.dev/api/sqlite/search", value);
    if (res.data) {
      if (res.data.count) {
