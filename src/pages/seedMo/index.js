...
const SearchForm = (props) => {
  ...
  return (
    <Form name="搜索" form={form} onFinish={onFinish} className="seed-form w-full">
      <SearchOne name="p1" />
      <div className="grid w-full grid-cols-4 " align="start">
        <div>
          {t("角色")}
...
const SeedCard = ({ data }) => {
  const [userId] = useAtom(uraraUserIdAtom);
  const [uraraDb] = useAtom(uraraDbAtom);
  ...
  return (
    <div className="w-1/2 p-1">
      <div className="flex flex-col border border-solid border-green-300">
        ...
        <div className="flex w-full items-center justify-around">
          {player && (
            <img alt={player.name} src={CDN_SERVER + player.imgUrl} className="h-10 w-10" />
          )}
          {support && <img alt={support.name} src={CDN_SERVER + support.imgUrl} className="w-10" />}
          <div className="flex text-lg" onClick={() => like(data)}>
            <SmileOutlined />
            <div>{data.likes}</div>
          </div>
          <div className="flex text-lg" onClick={() => dislike(data)}>
            <FrownOutlined />
