    ...
    },
    {
      ...
      render: (text, record) =>
        Object.keys(SEED_BLUE_LABELS).map((key) => {
          if (record[key]) {
            return <span className="rate-label">{`${SEED_BLUE_LABELS[key]}\xa0\xa0${record[key]}`}</span>;
          } else {
            return null;
          }
        }),
    },
    {
      ...
      render: (text, record) =>
        Object.keys(SEED_RED_LABELS).map((key) => {
          if (record[key]) {
            return <span className="rate-label">{`${SEED_RED_LABELS[key]}\xa0\xa0${record[key]}`}</span>;
          } else {
            return null;
          }
        }),
